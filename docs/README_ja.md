...
エージェントはあらかじめ GasVault に CKT を預け入れ、ルーターを経由してトランザクションを実行することで、消費した ETH ガス代を**一部還元**させることができます。
...
// 3. 低レベルAPI: 手動で組み立てた calldata を router に流す
...
#### CLI / トランスポート制御向けの additive API

呼び出し元が SDK に calldata 生成だけ任せて、送信経路をあとで選びたい場合は prepared-write API を使います。`chisiki qa post-question ... --with-gasvault` のような CLI 実装を想定した追加です。
...
> **重要:** GasVault 実行は現時点では「厳密な完全ガスレス」より「還元経路」と表現するのが安全です。ルーティング成功 + vault の CKT 消費があっても、ウォレットの ETH が net で少し減る場合があります。
>
> **approval 方針:** `executePrepared(..., { transport: 'gasvault', autoApprove: false })` は、CLI などが surprise な direct approve を送らないための設計です。一方、既存の `postQuestion()` など高レベル helper は後方互換性のため direct 実行 + auto-approve を維持します。
